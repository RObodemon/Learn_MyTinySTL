#ifndef TRYTINYSTL_VECTOR_H_
#define TRYTINYSTL_VECTOR_H_
...
namespace mystl
{
    ...
    public:
        ...
        // initializer_list: 提供访问元素数组的权限，其中数组的每个成员均具有指定的类型。
        vector& operator=(std::initializer_list<value_type> ilist)
        {
            // 定义 tmp 获取对象，然后swap到左值对象
    ...
    void vector<T>::try_init() noexcept
    {
        try
        {
            // data_allocator: mystl::allocator<T>的包装，这里是使用 operator new 分配内存
    ...
    {
        // size_type: size_t: 类似于 unsigned int，一般用来指明数组长度，有足够的大小来来存储可能的数组
    ...
    void vector<T>::destroy_and_recover(iterator first, iterator last, size_type n)
    {
        ...
        // deallocate负责释放内存
        //（此时相应内存中的值在此之前应调用destory销毁，将内存地址返回给系统，代表这部分地址使用引用 - 1）
    ...
    vector<T>& vector<T>::operator=(const vector& rhs)
    {
        if (this != &rhs)
        {
            ...
            if (len > capacity())
            {
                // 直接进行首尾迭代器交换, 这里应该改变capacity()的大小吧
                vector tmp(rhs.begin(), rhs.end());
                swap(tmp);
            }
            else if (size() >= len)
            {
                // 直接拷贝, 然后删除剩余，并重新给更新end_
    ...
    void vector<T>::reserve(size_type n)
    {
        if (capacity() < n)
        {
            ...
            // 把之前的空间都释放掉，然后更新 begin_, end_, cap_ 到新的空间中去
