#ifndef TRYTINYSTL_BASIC_STRING_H_
#define TRYTINYSTL_BASIC_STRING_H_
...
namespace mystl
{
	...
	struct char_traits
	{
		...
		// 把src中的值放到dst中，const 确保不改变src的值
		static char_type* copy(char_type* dst, const char_type* src, size_t n)
		{
			// 如果结果为 false，则打印诊断消息并中止程序。见 exceptdef.h
		...
		// 当dst的地址小于src的地址后，直接一个一个赋值
		// 当dst的地址大于 src的地址后，从第n个值开始赋值
	...
	struct char_traits<char>
	{
		...
		static char_type* copy(char_type* dst, const char_type* src, size_t n) noexcept
		{
			MYSTL_DEBUG(src + n <= dst || dst + n <= src);
			// memcpy: 在缓冲区之间复制字节
			// 不太理解这里为什么要使用 static_cast 进行类型转换
	...
	public:
		...
		// 普通旧数据(POD)类型也是旧 C 语言中的那些类型。 POD 类型还包括标量类型。
		// 返回true or false
		// static_assert: 如果前一句表达式为false: std::is_pod<CharType>::value 为false
		// 编译器会显示后面这句消息
	...
	public:
		...
		const_iterator begin() const noexcept // const 限制不对类对象的成员做出改变
		{
			return buffer_;
		}
		// end
	...
	// init_from 函数: 从位置 pos 处开始初始化到string中去
	template <class CharType, class CharTraits>
	void basic_string<CharType, CharTraits>::
	...
	{
		*(buffer_ + size_) = value_type();// 这个表达式？
		return buffer_;
	}
	...
	// reinsert 函数，shurink_to_fit 的辅助函数
	template <class CharType, class CharTraits>
	...
	// reallocate_and_fill 函数，insert 的辅助函数
	template <class CharType, class CharTraits>
	...
	// reallocate_and_copy 函数，inset 的辅助函数
	template <class CharType, class CharTraits>
	typename basic_string<CharType, CharTraits>::iterator
		basic_string<CharType, CharTraits>::
		...
		insert(const_iterator pos, Iter first, Iter last)
	{
		...
		char_traits::move(r + len, r, len);// else if 倒着赋值
	...
	// reallocate 函数： append的辅助函数， 
	template <class CharType, class CharTraits>
	void basic_string<CharType, CharTraits>::
		reallocate(size_type need)
	{
		// 右移除以二，判断所需空间后重新创建并进行更新
		...
		basic_string<CharType, CharTraits>::erase(const_iterator first, const_iterator last)
	{
		...
		// 把last 往后的全部移动到r后面，这里有个问题，如果 n>(last - first) ？ 所以需要更新size的值
		...
		basic_string<CharType, CharTraits>::find(const_pointer str, size_type pos) const noexcept
	{
		...
		const auto left = size_ - len; // 在第一个相同的字符一定要在 left 前出现
	...
	// 从下标 pos 开始查找字符串 s: 和上面的函数基本相同，重载提供了查找长度
	template <class CharType, class CharTraits>
	typename basic_string<CharType, CharTraits>::size_type
		basic_string<CharType, CharTraits>::
