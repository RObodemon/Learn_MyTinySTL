#ifndef TRYTINYSTL_FUNCTIONAL_H_
#define TRYTINYSTL_FUNCTIONAL_H_
...
namespace mystl
{
	...
	// #define----#undef
	// 进行宏定义，终止宏定义(终止宏的作用域)
	// 宏定义的功能是用一个标识符来表示字符串，标识符称为宏名。
	// 在预编译处理时，对程序中出现的宏名，在程序中出现的宏名，都用宏定义中出现的字符串去代替。
	// 这里是有参宏定义，反斜杠的作用是告诉编译器这里需要换行(在宏定义和printf() 中应用较多)
	...
	// 任意数量的 #elif 指令可以出现在 #if 和 #endif 指令之间，但最多允许一个 #else 指令。 
	// #else 指令（如果有）必须是 #endif 之前的最后一个指令。
	// 对于浮点数，逐位哈希
