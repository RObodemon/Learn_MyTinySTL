#ifndef TRYTINYSTL_LIST_H_
#define TRYTINYSTL_LIST_H_
...
namespace mystl
{
	...
	struct list_iterator :public mystl::iterator<mystl::bidirectional_iterator_tag, T>
	{
		...
		list_iterator(node_ptrx)::node_(x->as_base())
		{
		}
	...
	public:
		...
		void assign(size_type n, cosnt value_type& value)
		{
			fill_assign(n, value);
		}
	...
	// 在尾部连接 [first, last] fill_init, copy_init的辅助函数, emplace_back 的辅助函数
	template <class T>
	...
	void list<T>::copy_init(Iter first, Iter last)
	{
		...
		{
			clear();
			base_allocator::deallacate(node_);
			node_ = nullptr;
			throw;
		}
	}
	...
	// 在头部连接 [first, last] 结点： emplace_front 的辅助函数
	template <class T>
	...
	// 在 pos 处插入 [first, last) 的元素: insert 的辅助函数
	template <class T>
	template <class Iter>
	typename list<T>::iterator
	...
	// 将另一元操作 pred 为 true 的所有元素移除: remove的辅助函数
	template <class T>
	template <class UnaryPredicate>
	...
	// 对 list 进行归并排序，返回一个迭代器指向区间最小元素的位置: sort的辅助函数
	// ?
	template <class T>
	template <class Compared>
	typename list<T>::iterator
