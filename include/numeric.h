#ifndef TRYTINYSTL_NUMERIC_H_
#define TRYTINYSTL_NUMERIC_H_

// 数值计算
// accumulate: 累加算法 (重载 函数对象)
// adjacent_difference: 计算相邻差值 (重载，函数对象)
// inner_product: 计算两个区间的积 (重载，两个函数对象)
// iota: 递增
// partial_sum: 局部累计求和，并保存 (重载，函数对象)
#include"iterator.h"
...
namespace mystl
{
	...
	T accumulate(InputIter first, InputIter last, T init, BinaryOp binaryOp)
	{
		for (; first != last; ++first)
		{
