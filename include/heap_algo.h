#ifndef TRYTINYSTL_HEAP_ALGO_H_
#define TRYTINYSTL_HEAP_ALGO_H_
...
namespace mystl
{
	...
	// 最小堆：根节点小于左右两个子节点的完全二叉树 堆可以用数组来存储， 
	// a[i]处存根节点，a[2 * i]存左子树的根节点 ; 这里的 i 不从 0 开始
	// a[2 * i + 1]存右子树的根节点 https://www.pianshen.com/article/8949106919/
	...
	{
		// 先进行下溯（precolate down）过程
		auto topIndex = holeIndex; // 0
	...
	{
		result = *first; // 先将尾值设置成首值，即尾值为欲求结果
	...
	{
		// 每执行一次 pop_heap, 最大的元素都被放到尾部，知道容器最多只有一个元素，完成排序
		while (last - first > 1)
		{
	...
	{
		mystl::make_heap_aux(first, last, distance_tyhpe(first), comp);
	}
}
